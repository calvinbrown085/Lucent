...
App Store release: `scripts/release.sh [tvos|ios|all] --notes FILE [--build N] [--no-submit] [--check] [--dry-run]` runs `testflight.sh`, then `scripts/asc-release.swift` (App Store Connect API, ES256 JWT via CryptoKit — needs the `ASC_*` key with **App Manager** role) waits for processing, answers export compliance, creates/reuses the `appStoreVersions` record for `MARKETING_VERSION`, writes `whatsNew` to the en-US localization, attaches the build and submits a `reviewSubmission`. A notes file may hold `== tvos` / `== ios` sections. Refuses a dirty working tree; on success tags `<platform>/v<version>` locally. `scripts/release-notes-draft.sh [tvos|ios]` prints the commits/files since that tag as raw material for "What's New" (Claude writes the notes; keep iOS-only changes like PiP / docked player out of tvOS notes and vice versa).

Versioning: tvOS 1.1 (build 1.010) and iOS 1.0 were submitted 2026-09-14. From here every store version on either platform is **1.2 or higher**; TestFlight builds reuse the marketing version with the timestamp build number.
